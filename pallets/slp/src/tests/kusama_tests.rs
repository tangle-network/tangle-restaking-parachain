...
fn decrease_token_pool_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		tangle_lst_minting::TokenPool::<Runtime>::insert(KSM, 100);
...
fn update_ongoing_time_unit_works() {
	ExtBuilder::default().build().execute_with(|| {
		System::set_block_number(1);
		// Set the era to be 8.
		tangle_lst_minting::OngoingTimeUnit::<Runtime>::insert(KSM, TimeUnit::Era(8));
...
fn charge_host_fee_and_tune_lst_exchange_rate_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		tangle_lst_minting::OngoingTimeUnit::<Runtime>::insert(KSM, TimeUnit::Era(1));
